...
async function Get_Embedding_Question(Context) {
    const Embedding_Response = await fetch('http://localhost:11434/api/embeddings', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'bge-m3', prompt: Context })
    });
        ...
        while (Attempt_Count < Sending_Retires_Quota && !Vaild_Response) {
            ...
            const Response = await fetch('http://localhost:11434/api/chat', {
                ...
                body: JSON.stringify({
                    ...
                    stream: false,
                    options: { temperature: 0.6 + (Attempt_Count * 0.1) }
                })
            });

            if (!Response.ok) throw new Error("Network response has a problem");
            const data = await Response.json();

            Bot_Context_Reply = data.message.content;
